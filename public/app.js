angular
  ...
  .controller("TodoListController", function($scope, $http) {
    ...
    $http
      .get("https://sheetsu.com/apis/v1.0su/a5aa9216ff4d")
      .then(function(res) {
    ...
    setInterval(function() {
      ...
      $scope.autoTime = function() {
        ...
        $http
          .put("/setsys/" + $scope.totalsys[0]._id, $scope.sys[0])
          .then(res => {
    ...
    $scope.autotime = function() {
      ...
      if ($scope.time1 > timeStartStr && $scope.time1 < timeEndStr) {
        ...
        $http
          .put("/btn/" + $scope.totalbtn[0]._id, $scope.stabtn[0])
          .then(res => {
    ...
    if ($scope.staPump === true) {
      ...
      console.log("pumpOn");
      $http
        .put("/btn/" + $scope.totalbtn[1]._id, $scope.stabtn[1])
        .then(res => {
    ...
    if ($scope.staFog === true) {
      ...
      console.log("fogOn");
      $http
        .put("/btn/" + $scope.totalbtn[2]._id, $scope.stabtn[2])
        .then(res => {
    ...
    if ($scope.staWater === true) {
      ...
      console.log("waterOn");
      $http
        .put("/btn/" + $scope.totalbtn[3]._id, $scope.stabtn[3])
        .then(res => {
    ...
    setInterval(function() {
      ...
      $scope.daysDiffFunc = function(d1) {
        ...
        var HoursDiff =
          (timeDiff - Math.floor(DaysDiff) * (1000 * 3600 * 24)) /
          (1000 * 3600);
        var MinsDiff =
          (timeDiff -
            Math.floor(HoursDiff) * (1000 * 3600) -
            Math.floor(DaysDiff) * (1000 * 3600 * 24)) /
          (1000 * 60);
        return {
      ...
      $scope.tree = function(hole) {
        ...
        if ($scope.totalHole[index].status === false) {
          ...
          if (cur >= 0) {
            ...
            $http
              .put("/dateend/" + $scope.totalHole[cur]._id, $scope.hole[index])
              .then(res => {
                // console.log('th' +res)
      ...
      $scope.reday = function(list) {
        ...
        $http.get("/dateend").then(res => {
          ...
          $scope.hole[list].end = moment()
            .add(36, "days")
            .calendar();
          $http
            .put("/dateend/" + $scope.totalHole[list]._id, $scope.hole[list])
            .then(res => {
    ...
    setInterval(function() {
      ...
      function getdatahole() {
        $http.get("/hole").then(res => {
          if (
            JSON.stringify($scope.totaldatahole) !== JSON.stringify(res.data)
          ) {
      ...
      $scope.hole = function(id, veg) {
        ...
        if ($scope.totaldatahole[id]) {
          ...
          if (
            (veg == null && $scope.totaldatahole[id].statushole) ||
            (veg !== null && !$scope.totaldatahole[id].statushole)
          )
            $http
              .put("/hole/" + $scope.totaldatahole[id]._id, obj)
              .then(res => {
